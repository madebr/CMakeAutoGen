...
function(read_definition DEFINITION_FILENAME TEMPLATE_FILENAME DATA)
    ...
    while (1)
        ...
        while (1)
            string(REGEX MATCH "^([a-zA-Z_][a-zA-Z0-9_]*)[${WS}]*=[${WS}]*((\"([${WS}a-zA-Z0-9_\\][${WS}a-zA-Z0-9_%\\\"]*)\")|([a-zA-Z_\\][a-zA-Z0-9_%\\]*))[${WS}]*;[${WS}]*" KEY_VALUE_MATCH "${DEFINITION_CONTENTS}")
...
function(match_autogen_group TEXT START POS0 POS1 MATCH FOUND)
    ...
    string(REGEX MATCH "\\[\\+[${WS}]*([ =a-zA-Z0-9_\\(\\)\\\")]+)[${WS}]*\\+\\]" MATCH_GROUP "${TEXT}")
...
function(append_output_text SUFFICES_FILENAMES TEXT_APPEND FILTER)
    ...
    foreach(INDEX RANGE 0 ${NB_END} 2)
        ...
        if (WRITE_OK)
            ...
            file(APPEND "${FILENAME}" "${TEXT_APPEND}")
        endif()
    endforeach()
endfunction()
...
function(parse_template TEMPLATE_FILENAME TEMPLATE_PARAMETERS)
    ...
    foreach(SUFFIX ${SUFFICES})
        set(FILENAME "${TEMPLATE_NAME_WE}.${SUFFIX}")
        list(APPEND SUFFICES_FILENAMES "${SUFFIX}" "${FILENAME}")
        file(WRITE "${FILENAME}" "")
    endforeach()
    if (DEBUG)
    ...
    while (1)
        ...
        elseif (GROUP_MATCH MATCHES "^ENDFOR")
            ...
            if (NOT "${FOR_KEY}" STREQUAL "${ENDFOR_KEY}")
                message("FOR and ENDFOR do not match. (FOR:${FOR_KEY}, ENDOR:${ENDFOR_KEY})")
            endif()
        ...
        elseif (GROUP_MATCH MATCHES "^ESAC")
            ...
            string(REGEX MATCH "\\(([a-zA-Z_\\\"${WS}]+)\\)" PARENTHESE_MATCH "${GROUP_MATCH}")
...
parse_template("${TEMPLATE_FILENAME}" "${DATA}")
